...
For very long tracking experiments, it may be convenient to split a single experiment across multiple WCON files.  To make it easier to reconstruct tracks across files, we support a `files` entry in the main WCON object. There three fields, `current`, `next`, and `prev`, subject to the following rules:

1. The name of the file being parsed is in `current` (as a string).
3. If there is a next file, at least the next file must be listed under `next`; an array of following files is permitted.
4. If there is a previous file, at least the previous file must be listed under `prev`; earlier files may be listed also (as strings in an array) in reverse order.
...
{
    "files":{
        "current":"filename_2.wcon",
        "prev":["filename_1.wcon", "filename_0.wcon"],
        "next":["filename_3.wcon"]
     },
    "units":{"t":"s", "x":"mm", "y":"mm"},
    "data":{
