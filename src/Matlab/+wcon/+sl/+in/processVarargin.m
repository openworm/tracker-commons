...
if populate_extras_flag
    extras = wcon.sl.in.process_varargin_result(in,v);
else
    extras = [];
end
...
elseif iscell(v) && length(v) == 1 && isempty(v{1})
    ...
    parse_input = true;
    v = wcon.sl.in.propValuePairsToStruct(v,'allow_spaces',c.allow_spaces);
end
...
if ~all(is_present)
    if c.allow_non_matches
        ...
        error(['Bad variable names given in input structure: ' ...
            '\n--------------------------------- \n %s' ...
            ' \n--------------------------------------'],...
            wcon.sl.cellstr.join(badVariables,'d',','))
    end
end
